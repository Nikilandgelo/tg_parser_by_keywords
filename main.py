...
keywords: list[str] = [keyword.strip() for keyword in getenv("KEYWORDS").split(",")]
...
async def main():
    ...
    async with client:
        logging.info(colored(f"Getting {len(url_channels)} channels...", "cyan"))
        try:
            channels: list[Channel] = await client.get_entity(url_channels)
            while True:
                logging.info(colored("Start searching for new messages!", "magenta"))
            ...
            logging.error(
                colored(f"Error fetching messages from {channels}:\n{e}", "red")
            )


if __name__ == "__main__":
    logging.basicConfig(
        level=logging.INFO, format="{asctime} - {levelname}:{message}\n", style="{"
    )
