...
url_channels: list[str] = [channel.strip()
                           for channel in getenv('CHANNELS_LINKS').split(',')]
keywords: list[str] = [keyword.strip()
                       for keyword in getenv('KEYWORDS').split(',')]
    ...
    async with client:
        ...
        try:
            ...
            while True:
                ...
                await proceed_new_messages(client, channels, keywords, bot,
                                           target_user_id)
                logging.info(
                    colored('Time to sleep...', 'cyan')
                )
                await sleep(300)
